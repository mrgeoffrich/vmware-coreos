...
export class EnvironmentTaskManager {
    ...
    public async ValidateEnvironment(environmentDefinition: EnvironmentDefinition, subnetConfiguration: SubnetDefinition) {
        ...
        try {
            ...
            for (let role of environmentDefinition.Machines) {
                for (var i = 0; i < role.Count; i++) {
                    ...
                    if (ipAddresses.length === 1) {
                        if (role.Name === 'etcd') {
                            let clusterHealth = await validate(ipAddresses[0], 'testuser', 'TechLogi1980', 'etcdctl cluster-health');
                        ...
                        if (role.Name === 'worker') {
                            let clusterHealth = await validate(ipAddresses[0], 'testuser', 'TechLogi1980', 'sudo systemctl is-active docker');
